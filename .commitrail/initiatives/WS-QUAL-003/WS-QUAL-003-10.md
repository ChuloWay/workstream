...
- `backend/tests/projects/submission_policy_mutations/`: passive row/port support
  ...
  behavior tests, with a package marker to prevent basename collisions during
  canonical collection. No module reaches 500 lines; tests target 75 lines and must
...
| Reserve claimed/pending/committed/mismatched; failed completion | `test_repository.py`: fresh records, bound query values and null-safe predicates, no swallowed errors |
...
Local: joint collection of this family and `projects/sufficiency_mutations` must
not collide on shared test basenames. Original controlled-port nodes, then the new controlled-port modules and
...
| `replay_repository_idempotency_classifies_cross_action_states` | `test_reservation_classifies_existing_exact_row`, `test_reservation_rejects_changed_operation_facts`, `test_reservation_rejects_different_operation_in_human_namespace`, `test_reservation_insert_binds_exact_values`, `test_namespace_query_binds_exact_selectors`, `test_operation_lookup_uses_exact_operation_predicate`, `test_completion_requires_exact_pending_row`; lookup-fixture correction in [11](WS-QUAL-003-11.md) |
...
losing its inserted row; a conflict with neither matching operation nor unique
namespace row under the supported reservation flow; caller-supplied random create-policy identity where the
...
Replay fixtures select persisted rows by their exact ID. The replay proof checks
the committed-policy lookup, including the predecessor lookup for update; an
unconditional mock return must not conceal a wrong selector. Warning fixtures
use either complete canonical acknowledgement custody or entirely null custody.
Partial acknowledgement and noncanonical action fixtures were pruned because
they violate the database shape constraint; foreign-project acknowledgement and
unacknowledged warnings retain distinct valid-state denial proofs. Ordinary
passed reports carry no warning acknowledgement custody.
