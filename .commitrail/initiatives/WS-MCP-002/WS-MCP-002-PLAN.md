...
live runtime proof. This proposal is reconciled with main `a3e4c696`; the API
handoff remains pinned to `6feef39834737eed106773fdaed6003561fd021a`.
...
- `WS-MCP-002-01.md`: the first implementation contract, drafted here without runtime code or completed runtime acceptance.
...
Use one initiative overview, linked above, and this planning change record.
The maintainer's next-step direction adds a ten-chunk map and a proposed first
implementation contract based on the current combined template. That contract
will be updated in its own implementation PR; no duplicate intent/risk/evidence
files are added. Each later implementation PR gets exactly one matching record.
...
- [x] All 27 tools have one chunk owner, with acyclic dependencies and per-chunk evidence.
- [x] The first contract defines one observable protected read and puts credential/schema decisions before dependent code.
...
| --- | --- | --- | --- |
| Current process followed | CONTRIBUTING.md, Commitrail template and planning skill inspection | One overview, planning record, first-contract draft and index row | Human agreement remains on the PR |
...
| Changes remain planning-only | `git diff --stat origin/main`; `git diff --check origin/main` | PASS: planning/navigation changes only; no whitespace errors | No runtime execution is claimed |
...
| Markdown links resolve | `python3 scripts/check_markdown_links.py` | PASS: four changed Markdown files checked | Does not prove remote service availability |
...
The maintainer's [bounded-work direction](https://github.com/Flow-Research/workstream/pull/401#issuecomment-5655130650)
replaces four broad stages with coherent, separately tested PR boundaries.
The new map separates administrative reads, grants, actor lifecycle, project
shells, participation, guide metadata and policy writes. Only the first combined
contract is drafted now. Its runtime criteria remain unchecked; joint catalogue
freeze and credential agreement are not inferred from a positive review reaction.
...
- Current-source reconciliation: main `a3e4c696` is incorporated, preserving upstream index updates. The API handoff stays pinned to `6feef39834737eed106773fdaed6003561fd021a`. Relevant differences include project-role target lock ordering and additional post-submit policy surfaces; no new MCP endpoint is inferred. The mapped self-profile route/schema are unchanged. Recheck concurrent owner work before implementation.
...
- Next usable boundary: review the chunk map and first contract, complete pinned running-server schema capture/joint freeze and credential agreement, then implement only WS-MCP-002-01 in its own PR.
