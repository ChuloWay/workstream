...
| `test_resource_context.py::test_policy_tuple_and_transition_shape` | All three valid classifications produce exact allow evidence and receipts; pure service |
| `test_resource_context.py::test_ready_finalization_rejects_each_missing_policy_field` | Both ready classifications reject each independently missing policy ID, operation ID or digest while retaining every other valid fact; exact shape error, pure contract |
| `test_resource_context.py::test_blocked_finalization_rejects_each_populated_policy_field` | Blocked classification rejects each independently populated policy field using a valid UUID or digest; exact shape error, pure contract |
| `test_resource_context.py::test_finalization_rejects_conflicting_classification_outcome` | All three known classifications reject the opposite terminal outcome with their otherwise valid policy tuple intact; exact shape error, pure contract |
