...
| Covered PM reads the current exact draft target | `guide_manager_resource_denial`, `PostPolicyService.review_package` | `test_postgresql.py::test_manager_reads_exact_post_policy`: returned target equals the derived target, package is current, and the PM audit event has the exact resource and permission | PostgreSQL transaction with live AUTH |
| Review package includes the policy payload and proposal reference | `PostPolicyService.review_package` | Existing `backend/tests/projects/post_policy/test_postgresql.py::test_post_policy_operation_preserves_finalization_without_provider_or_evaluator_calls`: compiled current draft has the derived policy hash, eight policy entries and the original proposal target | PostgreSQL transaction with the product-owner authority fixture |
