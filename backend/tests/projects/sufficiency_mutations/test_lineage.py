...
@pytest.mark.parametrize("fault", ["run", "generation", "missing_row", "status", "step", "task"])
async def test_setup_custody_rejects_stale_context(case, fault):
    kwargs = custody_arguments(case)
    if fault == "run":
        kwargs["setup_run_id"] = UUID(int=99)
    elif fault == "generation":
        kwargs["setup_generation"] = 2
    elif fault == "missing_row":
        case.projects.lock_project_setup_run.return_value = None
    elif fault == "status":
        case.setup.status = "failed"
    elif fault == "step":
        case.setup.current_step = "enqueue"
    else:
...
async def test_setup_custody_resolves_current_context(case):
    kwargs = custody_arguments(case)
    result = await case.service.resolve_setup_service_custody(**kwargs)
    assert result.model_dump(mode="json") == {
        ...
        "correlation_id": str(UUID(int=21)),
