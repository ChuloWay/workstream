...
def test_binding_uses_registered_configuration():
    definition = catalogue().definition("check_acceptance_criteria_present", "v0.2")
    ...
    with pytest.raises(ValueError, match="unavailable"):
        altered_catalogue(state="disabled").definition(
            "check_acceptance_criteria_present", "v0.2"
        ).validate_configuration({})


@pytest.mark.parametrize(
    "name,version", [("unknown", "v0.2"), ("check_acceptance_criteria_present", "v0.1")]
)
