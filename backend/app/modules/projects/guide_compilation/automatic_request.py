...
from app.interfaces.artifact_operations import GuideSufficiencyMaterialPort, GuideSufficiencyMaterialRequest
from app.interfaces.project_agents import PostSubmissionCapabilityProjection, PreSubmissionCapabilityProjection
from app.modules.authorization.api import ProjectGuideCompilationRequestFacts, ProjectGuideCompilationRequestOrigin
from app.modules.projects.models import GuideMutationIdempotencyRecord, GuideSourceSnapshot, ProjectGuide, ProjectSetupRun
...
class AutomaticCompilationInputs:
    ...
    async def resolve(
        self, session: AsyncSession, setup_run_id: UUID,
    ) -> tuple[ProjectGuideCompilationRequestFacts, CompilationAttemptIdentity, ProjectGuideCompilationRequestOrigin]:
        ...
        mutation = await session.scalar(select(GuideMutationIdempotencyRecord).where(
        ...
        if guide is None or snapshot is None or mutation is None or setup.authorization_decision_event_id is None:
        ...
        loaded = await self.material.load(GuideSufficiencyMaterialRequest(
            project_id=UUID(setup.project_id), guide_id=UUID(setup.guide_id),
            guide_source_snapshot_id=UUID(setup.source_snapshot_id),
            project_setup_run_id=setup_run_id, setup_generation=setup.setup_generation,
        ))
        context = compilation_context_from_material(
            guide=guide, snapshot=snapshot, loaded=loaded,
            setup_run_id=setup_run_id, setup_generation=setup.setup_generation,
        ...
        predecessor = await GuideCompilationRepository(session).current_compilation(
            UUID(setup.project_id), UUID(setup.guide_id), lock=False,
        )
        facts = ProjectGuideCompilationRequestFacts(
            **identity.model_dump(), operation_id=operation_id,
            request_id=uuid5(operation_id, "request"), idempotency_key=uuid5(operation_id, "idempotency"),
...
def automatic_operation_id(setup_run_id: UUID, generation: int) -> UUID:
    ...
    return uuid5(NAMESPACE_URL, f"workstream.project.guide_compilation.automatic:{setup_run_id}:{generation}")
