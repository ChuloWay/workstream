...
class PreSubmitAttemptClaim:
    ...
    async def _reserved(
        ...
        *,
        ...
        statement = (
            select(PreSubmitExecutionAttempt)
            .where(
        ...
        if (
            ...
            or row.evidence_set_id is not None
            or row.claim_nonce != str(self._nonce)
            or row.request_digest != self.request_digest
    ...
    async def validate_completion(
        self,
        session: AsyncSession,
        request: PreSubmitEvidencePersistenceRequest,
    ) -> None:
    ...
    async def complete(self, session: AsyncSession, evidence_id: UUID) -> None:
        ...
        result = await session.execute(
            update(PreSubmitExecutionAttempt)
            .where(
                PreSubmitExecutionAttempt.id == str(self.attempt_id),
...
class PreSubmitAttemptStore:
    ...
    async def reserve(
        ...
        *,
        ...
        if inserted is not None:
            self._pending = _Reservation(
                attempt_id, nonce, digest, replace(request, prepared_authorization=None)
            )
    ...
    async def committed_claim(self, reservation: object) -> PreSubmitAttemptClaim:
        ...
        async with self._session.begin():
            row = await self._session.get(
                PreSubmitExecutionAttempt, str(reservation.attempt_id), populate_existing=True
            )
            if (
                row is None
                or row.claim_nonce != str(reservation.claim_nonce)
                or row.request_digest != reservation.request_digest
                or row.status != "reserved"
            ):
                raise PreSubmitEvidenceConflict("pre_submit_attempt_not_committed")
    ...
    async def read_completed(
        self,
        *,
        row: PreSubmitExecutionAttempt,
        context: PreSubmitEvidenceContext,
        plan: EffectivePreSubmissionExecutionPlan,
    ) -> PreSubmitEvidencePersistenceResult:
        """Return original evidence after caller revalidates original fixed-service facts."""
        evidence = await self._session.get(
            PreSubmitEvidenceSet, row.evidence_set_id, populate_existing=True
        )
        if (
            evidence is None
            or row.status != "completed"
            or evidence.attempt_id != row.id
        ...
        if len(rows) != len(plan.entries) or any(
            r.metadata_json is None or r.checker_order is None for r in rows
        ):
        ...
        execution = PreSubmitExecutionResult(
            ...
            checker_facts=PreSubmissionExecutionFacts(
                plan_sha256=evidence.effective_plan_sha256,
                eligible=evidence.eligible,
                entries=tuple(
                    PreSubmissionExecutionEntryFacts(
                        dispatch_authority=r.dispatch_authority,
                        definition_id=r.definition_id,
                        definition_version=r.definition_version,
                        public_name=r.public_name,
                        policy_source=r.source,
                        effective_plan_sha256=r.effective_plan_sha256,
                        rule_instance_id=r.rule_instance_id,
                        locked_policy_sha256=r.locked_policy_sha256,
                        phase=r.phase,
                        order=r.checker_order,
                        classification=r.classification,
                        severity=r.severity,
                        checker_execution_status=r.status,
                        failure_code=r.failure_code,
                        message_code=r.message_code,
                        metadata=tuple(tuple(item) for item in r.metadata_json),
                    )
                    for r in rows
                ),
            ),
        ...
        identity = original.operation_identity(
            effective_plan_sha256=plan.plan_sha256,
        ...
        for ordinal, (member, entry) in enumerate(zip(rows, execution.entries, strict=True)):
            values = _PreSubmitEvidenceRepository._result_values(
                ordinal, entry, plan.entries[ordinal].result_schema
            )
        ...
        failure_audit = (
            ...
            if execution.eligible
                ...
                actor_profile_id=original.actor_profile_id,
