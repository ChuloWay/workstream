...
def guide_manager_resource_denial(action, resource, project_id):
    ...
    expected = (
        ...
        if action in POST_POLICY_HUMAN_ACTION_IDS and type(resource) is PostPolicyResourceContext
        else (
    ...
    if (
        action in GUIDE_PROPOSAL_ACTION_IDS | POST_POLICY_HUMAN_ACTION_IDS
        and resource.facts.locator.action_id != action
    ):
        return AuthorizationDenialCode.RESOURCE_GUARD_DENIED
