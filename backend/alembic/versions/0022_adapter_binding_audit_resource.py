"""Admit exact AdapterBinding decision resources without widening audit facts."""

from alembic import op
import sqlalchemy as sa

revision = "0022_adapter_binding_audit_resource"
down_revision = "0021_pre_submit_attempts"
branch_labels = None
depends_on = None

_ANCHOR = "('contribution_policy'::character varying)::text"
_RESOURCE = "('compensation_adapter_binding'::character varying)::text"
_TOKEN = ", " + _RESOURCE
_CONSTRAINT = "ck_audit_events_authority_privacy_bounds"
_ACTION_CONSTRAINT = "ck_audit_events_authorization_action_evidence"
_ACTIONS = tuple("compensation.adapter_binding." + operation for operation in (
    "read", "create", "suspend", "resume"
))
_ACTION_ANCHOR = "(((action_id)::text = 'actor.profile.read_self'::text) AND ((permission_id)::text = 'actor.profile.read_self'::text))"
_ACTION_PAIRS = tuple(
    "(((action_id)::text = '" + action + "'::text) AND ((permission_id)::text = 'compensation.adapter_binding.manage'::text))"
...
def _amend(*, add: bool) -> None:
    ...
    if (
        ...
        and connection.execute(
            sa.text(
                "select exists(select 1 from audit_events where resource_type='compensation_adapter_binding' "
                "or action_id in ('compensation.adapter_binding.read','compensation.adapter_binding.create',"
                "'compensation.adapter_binding.suspend','compensation.adapter_binding.resume'))"
            )
        ).scalar_one()
    ):
        raise RuntimeError("AdapterBinding audit history prevents downgrade")
...
def upgrade() -> None:
    """Allow only the exact binding token in the existing closed resource vocabulary."""
    _amend(add=True)


def downgrade() -> None:
    """Refuse loss of binding evidence; otherwise restore the exact prior constraint."""
    _amend(add=False)
